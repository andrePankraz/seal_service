...
import de.init.seal_service.pdf.pdfbox_signature.cert.CertificateVerifier;

/**
 * This class helps to extract data/information from a signature. The
 * information is held in CertSignatureInformation. Some information is needed
 * for validation processing of the participating certificates.
 *
 * @author Alexis Suter
 *
 */
public class CertInformationCollector {
	/**
	 * Data class to hold Signature, Certificate (and its chain(s)) and revocation
	 * Information
	 */
	...
	/**
	 * Adds multiple Certificates out of a Collection of X509CertificateHolder into
	 * certificateSet.
	 *
	...
	private void addAllCerts(final Collection<X509CertificateHolder> certHolders) {
		for (final X509CertificateHolder certificateHolder : certHolders) {
			try {
				final var certificate = getCertFromHolder(certificateHolder);
	...
	/**
	 * Gets a list of X509Certificate out of an array of X509CertificateHolder. The
	 * certificates will be added to certificateSet.
	 *
	 * @param certHolders Array of X509CertificateHolder
	 * @throws CertificateProccessingException when one of the Certificates could
	 *                                         not be parsed.
	 */
	...
	/**
	 * Processes an embedded signed timestamp, that has been placed into a
	 * signature. The certificates and its chain(s) will be processed the same way
	 * as the signature itself.
	 *
	 * @param signerInformation of the signature, to get unsigned attributes from
	 *                          it.
	 * @throws IOException
	 * @throws CertificateProccessingException
	 */
	private void addTimestampCerts(final SignerInformation signerInformation)
		...
		final var unsignedAttributes = signerInformation.getUnsignedAttributes();
		if (unsignedAttributes == null) {
			return;
		}
		final var tsAttribute = unsignedAttributes.get(PKCSObjectIdentifiers.id_aa_signatureTimeStampToken);
		if (tsAttribute == null) {
			return;
		}
		final var obj0 = tsAttribute.getAttrValues().getObjectAt(0);
		if (!(obj0 instanceof final ASN1Object tsSeq)) {
			return;
		}
		try {
			final var signedData = new CMSSignedData(tsSeq.getEncoded("DER"));
			this.rootCertInfo.tsaCerts = new CertSignatureInformation();
			processSignerStore(signedData, this.rootCertInfo.tsaCerts);
	...
	/**
	 * Get alternative certificate chain, from the Authority Information (a url). If
	 * the chain is not included in the signature, this is the main chain. Otherwise
	 * there might be a second chain. Exceptions which happen on this chain will be
	 * logged and ignored, because the cert might not be available at the time or
	 * other reasons.
	 *
	 * @param certInfo base Certificate Information, on which to put the alternative
	 *                 Certificate
	 * @param maxDepth Maximum depth to dig through the chain from here on.
	 * @throws CertificateProccessingException on a specific error with a
	 *                                         certificate
	 */
	private void getAlternativeIssuerCertificate(final CertSignatureInformation certInfo, final int maxDepth)
		...
		try {
			...
			try (var in = SigUtils.openURL(certInfo.issuerUrl)) {
				final var altIssuerCert = (X509Certificate) certFactory.generateCertificate(in);
	 ...
	 * @return a X509Certificate or <code>null</code> when there was an Error with
	 *         the Certificate
	 * @throws CertificateProccessingException on failed conversion from
	 *                                         X509CertificateHolder to
	 *                                         X509Certificate
	 */
	...
	private CertSignatureInformation getCertInfo(final byte[] signatureContent)
		...
		try {
			final var signedData = new CMSSignedData(signatureContent);
			final var signerInformation = processSignerStore(signedData, this.rootCertInfo);
	...
	CertSignatureInformation getCertInfo(final X509Certificate certificate) throws CertificateProccessingException {
		try {
			final var certSignatureInformation = new CertSignatureInformation();
	 ...
	 * @throws CertificateProccessingException when there is an error processing the
	 *                                         certificates
	 * @throws IOException                     on a data processing error
	 */
	public CertSignatureInformation getLastCertInfo(final PDSignature signature, final byte[] inPdf)
			throws CertificateProccessingException, IOException {
		try (var documentInput = new ByteArrayInputStream(inPdf)) {
			final var signatureContent = signature.getContents(documentInput);
	...
	public CertSignatureInformation getLastCertInfo(final PDSignature signature, final String fileName)
			throws CertificateProccessingException, IOException {
		try (var documentInput = new FileInputStream(fileName)) {
			final var signatureContent = signature.getContents(documentInput);
			return getCertInfo(signatureContent);
		}
	}
	...
	/**
	 * Processes a signer store and goes through the signers certificate-chain. Adds
	 * the found data to the certInfo. Handles only the first signer, although
	 * multiple would be possible, but is not yet practicable.
	 *
	 * @param signedData data from which to get the SignerInformation
	 * @param certInfo   where to add certificate information
	 * @return Signer Information of the processed certificatesStore for further
	 *         usage.
	 * @throws IOException                     on data-processing error
	 * @throws CertificateProccessingException on a specific error with a
	 *                                         certificate
	 */
	private SignerInformation processSignerStore(final CMSSignedData signedData,
			final CertSignatureInformation certInfo) throws IOException, CertificateProccessingException {
		final var signers = signedData.getSignerInfos().getSigners();
		final var signerInformation = signers.iterator().next();

		final var certificatesStore = signedData.getCertificates();
		@SuppressWarnings("unchecked")
		final Collection<X509CertificateHolder> matches = certificatesStore.getMatches(signerInformation.getSID());

		final var certificate = getCertFromHolder(matches.iterator().next());
		this.certificateSet.add(certificate);

		final var allCerts = certificatesStore.getMatches(null);
	...
	/**
	 * Traverse through the Cert-Chain of the given Certificate and add it to the
	 * CertInfo recursively.
	 *
	 * @param certificate Actual Certificate to be processed
	 * @param certInfo    where to add the Certificate (and chain) information
	 * @param maxDepth    Max depth from this point to go through CertChain (could
	 *                    be infinite)
	 * @throws IOException                     on data-processing error
	 * @throws CertificateProccessingException on a specific error with a
	 *                                         certificate
	...
	private void traverseChain(final X509Certificate certificate, final CertSignatureInformation certInfo,
		...
		final var authorityExtensionValue = certificate.getExtensionValue(Extension.authorityInfoAccess.getId());
		...
		final var crlExtensionValue = certificate.getExtensionValue(Extension.cRLDistributionPoints.getId());
		...
		for (final X509Certificate issuer : this.certificateSet) {
			try {
		...
		if (certInfo.issuerCertificate == null) {
			throw new IOException("No Issuer Certificate found for Cert: '" + certificate.getSubjectX500Principal()
					+ "', i.e. Cert '" + certificate.getIssuerX500Principal() + "' is missing in the chain");
		}
	}
}
